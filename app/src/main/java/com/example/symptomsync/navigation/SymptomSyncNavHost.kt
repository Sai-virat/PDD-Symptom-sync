...
fun SymptomSyncNavHost(navController: NavHostController) {
    ...
    NavHost(navController = navController, startDestination = Screen.Splash) {
        ...
        composable<Screen.CoreDietPlan> { 
            CoreDietPlanScreen(coreViewModel, onMealClick = { type, symptom -> navController.navigate(Screen.CoreMealDetail(type, symptom)) }) 
        }
        composable<Screen.CoreMealDetail> { backStackEntry ->
            val route = backStackEntry.toRoute<Screen.CoreMealDetail>()
            CoreMealDetailScreen(route.mealType, route.symptomName, coreViewModel, onBack = { navController.popBackStack() })
        }
